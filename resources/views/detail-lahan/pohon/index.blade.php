<x-main-layout>
    <x-slot name="header">
        <div class="flex flex-row items-center justify-between gap-4">
            <h2 class="text-xl font-bold text-darkslategray font-outfit">Data Pohon</h2>
            <div class="self-stretch flex flex-row items-center justify-start gap-1.5 text-left text-sm text-slategray font-outfit">
                ...
                <div class="relative leading-5 text-darkslategray-200 font-medium">Data Pohon</div>
            </div>
        </div>
    </x-slot>
                    ...
                    <div class="w-full flex flex-col items-start justify-start shadow-sm rounded-2xl">
                        <div class="rounded-t-2xl bg-white border-gainsboro border-solid border-[1px] border-b-0 box-border self-stretch flex flex-row items-center justify-between gap-1 py-4 px-6">
                            ...
                            <div class="flex flex-row items-center justify-between gap-2">
                                <x-pohon.filter-button :lahan="$lahan" :jenisPohonList="$jenisPohonList"/>
                                <x-pohon.action-button :lahan="$lahan"/>
                            </div>
                        </div>
                        <x-pohon.list :lahan="$lahan" :pohon="$pohon" :hasFilter="$hasFilter" :tahunList="$tahunList"/>
                    </div>
                </div>
            </div>
        </div>
