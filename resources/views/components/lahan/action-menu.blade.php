...
<div class="relative" x-data="{ open: false }">
    ...
    <button 
        @click="open = !open" 
        class="flex border-none fill-none bg-transparent hover:bg-gray-200 rounded-lg p-2 items-center cursor-pointer"
    >
    ...
    <div 
        ...
        class="absolute z-10 mt-1 rounded-lg top-8 right-0 lg:right-auto bg-slategray-200 ring-black/5 focus:outline-hidden"
    >
        <div class="py-1">
            @can('viewTeam', $lahan)
                <a 
                    href="{{ route('lahan.team.index', $lahan->lahan_id) }}" 
                    class="no-underline text-left block px-3 py-2 text-sm text-white hover:bg-gray-200 font-outfit border-none bg-slategray-200 rounded-lg whitespace-nowrap"
                >
                    Access Management
                </a>
            @endcan
            ...
            @can('delete', $lahan)
                <button 
                    ...
                    class="w-full no-underline text-left block px-3 py-2 text-sm text-white hover:bg-gray-200 font-outfit border-none bg-slategray-200 rounded-lg"
                >
                    Delete
                </button>
            @endcan
