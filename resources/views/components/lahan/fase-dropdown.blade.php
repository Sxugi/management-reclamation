...
@php
    $options = \App\Models\Lahan::FASE;
    $modalName = 'confirm-fase-change-' . ($lahan->lahan_id ?? uniqid());
@endphp
...
<div x-data="{ 
    ...
    class="w-full relative flex flex-col items-start justify-start text-left text-sm">
    ...
    >
        <div class="flex-1 flex flex-row items-center justify-start text-white capitalize">
            <div class="flex-1 relative leading-5 font-medium font-outfit">
                {{ $fase }}
            </div>
        </div>
    ...
    <x-main.modal name="{{ $modalName }}" focusable>
        <form method="POST" action="{{ route('lahan.update-fase', $lahan ? $lahan->lahan_id : '') }}" class="p-6 text-left whitespace-normal">
            @csrf
            @method('PATCH')
            <input type="hidden" name="fase" :value="targetFase">

            <h2 class="text-lg font-medium text-gray-900">
                {{ __('Confirmation Phase Change') }}
            </h2>

            <p class="mt-1 text-sm text-gray-600">
                {{ __('Are you sure you want to change the phase of this land to') }} 
                <span class="font-bold text-darkslategray" x-text="targetFase"></span>?
            </p>
