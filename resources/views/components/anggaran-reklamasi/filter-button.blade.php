@props(['lahan', 'kategoriAnggaranList', 'tab_aktif'])
...
<div class="relative inline-block gap-2">
    <a class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200 cursor-pointer"
        ...
        onclick="toggleAnggaranFilterPanel()">
    ...
    <div id="anggaranFilterPanel" class="absolute right-0 z-50 mt-2 p-4 bg-white border border-gray-300 rounded-lg shadow-lg hidden w-96">
        <form id="anggaranFilterForm" method="GET" action="{{ route('lahan.anggaran.index', $lahan->lahan_id) }}" class="space-y-3">
            <input type="hidden" name="tab" value="{{ request()->query('tab', $tab_aktif) }}">
            <div class="flex justify-between gap-2">
                <div class="flex flex-col w-1/2">
                    <x-main.input-label for="startYear" class="block text-sm font-medium text-gray-700 mb-1">Tahun Awal</x-main.input-label>
            ...
            <div class="flex justify-between gap-2">
                <div class="flex flex-col w-1/2">
                    <x-main.input-label for="minNominal-display" class="block text-sm font-medium text-gray-700 mb-1">Nominal Min</x-main.input-label>
                    <x-main.text-input 
                        type="text" 
                        id="minNominal-display" 
                        placeholder="Rp. 0"
                        class="border rounded p-2 text-sm"
                        autocomplete="off"
                        inputmode="numeric"
                        value="{{ request('minNominal') ? 'Rp. ' . number_format(request('minNominal'), 0, ',', '.') : '' }}"
                    />
                    <input 
                        type="hidden" 
                        name="minNominal"
                        id="minNominal-input"
                        value="{{ request('minNominal') }}"
                    />
                </div>
                <div class="flex flex-col w-1/2">
                    <x-main.input-label for="maxNominal-display" class="block text-sm font-medium text-gray-700 mb-1">Nominal Max</x-main.input-label>
                    <x-main.text-input 
                        type="text" 
                        id="maxNominal-display" 
                        placeholder="Rp. 0"
                        class="border rounded p-2 text-sm"
                        autocomplete="off"
                        inputmode="numeric"
                        value="{{ request('maxNominal') ? 'Rp. ' . number_format(request('maxNominal'), 0, ',', '.') : '' }}"
                    />
                    <input 
                        type="hidden" 
                        name="maxNominal"
                        id="maxNominal-input"
                        value="{{ request('maxNominal') }}"
                    />
                </div>
            </div>
            ...
            <div>
                ...
                <x-main.text-input 
                    ...
                    autocomplete="off"
                    inputmode="numeric"
                    value="{{ request('minTotal') ? 'Rp. ' . number_format(request('minTotal'), 0, ',', '.') : '' }}"
                />
                <input 
                    type="hidden" 
                    name="minTotal"
                    id="minTotal-input"
                    value="{{ request('minTotal') }}"
                />
            </div>
            ...
            <div class="flex justify-between gap-2">
                <div class="flex flex-col w-1/2">
                    ...
                    <select id="startQuarter" name="startQuarter" class="text-sm block w-full border-solid border-[1px] border-gray-300 focus:border-darkslategray focus:ring-darkslategray rounded-md px-3 py-2 box-border font-outfit flex-1 leading-5 bg-transparent">
                        ...
                        <option value="Q3" @selected(request('startQuarter') === 'Q3')>Q3</option>
                        <option value="Q4" @selected(request('startQuarter') === 'Q4')>Q4</option>
                    </select>
                </div>
                <div class="flex flex-col w-1/2">
                    ...
                    <select id="endQuarter" name="endQuarter" class="text-sm block w-full border-solid border-[1px] border-gray-300 focus:border-darkslategray focus:ring-darkslategray rounded-md px-3 py-2 box-border font-outfit flex-1 leading-5 bg-transparent">
                        ...
                        <option value="Q3" @selected(request('endQuarter') === 'Q3')>Q3</option>
                        <option value="Q4" @selected(request('endQuarter') === 'Q4')>Q4</option>
                    </select>
                </div>
            </div>

            <div class="flex justify-between pt-2">
                <x-main.primary-button type="button" onclick="clearAnggaranFilters()" class="px-4 py-2">Clear</x-main.primary-button>
                <div class="flex gap-2">
                    <x-main.primary-button type="button" onclick="closeAnggaranFilterPanel()" class="px-4 py-2">Cancel</x-main.primary-button>
...
<script>
    ...
    function setupRupiahPair(displayId, hiddenId) {
        ...
        displayInput.addEventListener('input', function(e) {
            let numericValue = parseRupiah(e.target.value);
        ...
        if (form) {
            form.addEventListener('submit', function() {
                let numericValue = parseRupiah(displayInput.value);
                hiddenInput.value = numericValue && !isNaN(parseFloat(numericValue)) ? numericValue : '';
            });
        }
    }

    document.addEventListener('DOMContentLoaded', function() {
        setupRupiahPair('minNominal-display', 'minNominal-input');
        setupRupiahPair('maxNominal-display', 'maxNominal-input');
        setupRupiahPair('minTotal-display', 'minTotal-input');
    });
