...
<div class="w-full max-w-6xl mx-auto font-outfit">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <x-dokumentasi.open-modal :dokumentasi="$dokumentasi" />

        <div class="flex flex-col h-full">
            <div class="flex flex-col h-full">
                <div class="text-lg font-bold text-darkslategray-200 mb-2">Deskripsi</div>
                @if($dokumentasi->deskripsi)
                    <div class="h-full prose prose-sm max-w-none border border-gainsboro rounded-lg p-4 bg-white flex flex-col">
                        <p class="text-gray-700 leading-relaxed whitespace-pre-line flex-1">{{ $dokumentasi->deskripsi }}</p>
                    </div>
                @else
...
<div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 font-outfit">
    <a href="{{ route('lahan.dokumentasi.edit', [$lahan, $dokumentasi]) }}" 
        class="rounded-lg bg-darkslategray text-sm overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200">
    ...
    <x-main.primary-button x-data="" 
            ...
            class="py-3 px-4 gap-2 font-medium bg-red-600 hover:!bg-red-700 focus:bg-red-700 active:bg-red-800">
