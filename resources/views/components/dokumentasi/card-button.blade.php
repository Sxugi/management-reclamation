@php
    $modalData = [
        'dokumentasi_id' => $doc->dokumentasi_id,
        'nama' => $doc->nama,
        'deskripsi' => $doc->deskripsi,
        'image_path' => $doc->image_path ? Storage::url($doc->image_path) : null,
        'download_url' => $doc->image_path ? Storage::url($doc->image_path) : null,
        'created_at' => $doc->created_at->toISOString(),
        'updated_at' => $doc->updated_at->toISOString(),
        'edit_url' => route('lahan.dokumentasi.edit', [$lahan, $doc])
    ];
@endphp

<div class="flex flex-row items-center justify-center gap-3 p-4 mt-auto">
    <button type="button"
            onclick='window.openDokumentasiModal(@json($modalData))'
        class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-2.5 px-4 gap-2 !text-white border-none cursor-pointer hover:bg-slategray-200 cursor-pointer">
        <span class="text-leading-5 font-medium text-sm">Lihat Detail</span>
    </button>
    
    <div class="relative" x-data="{ open: false }">
        <button @click="open = !open" 
                class="flex border-none fill-none bg-darkslategray hover:bg-slategray-200 rounded-lg p-2.5 items-center cursor-pointer" 
                title="More Options">
        ...
        <div x-show="open" 
            ...
            <div class="py-1">
                <div class="inline">
                    <button onclick="window.location='{{ route('lahan.dokumentasi.edit', [$lahan, $doc]) }}'" 
                    class="w-full px-4 py-2 text-sm rounded-md !text-white hover:bg-slategray-200 flex items-center gap-2 no-underline cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    @disabled(!auth()->user()->can('update', $doc))>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            @if($doc->image_path)
                <div class="inline">
                    ...
                    download="{{ $doc->nama }}"
                    class="w-full px-4 py-2 text-sm rounded-md !text-white hover:bg-slategray-200 flex items-center gap-2 no-underline">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            @endif
                <button x-data="" 
                        ...
                        class="w-full px-4.5 py-2 text-sm rounded-md text-white hover:bg-slategray-200 flex items-center gap-2 no-underline font-outfit border-none bg-transparent cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" 
                        @disabled(!auth()->user()->can('delete', $doc))>
