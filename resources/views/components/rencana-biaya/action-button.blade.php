...
<div class="flex flex-row items-center justify-between gap-2 py-4 text-white">
    @if (request()->routeIs('lahan.rencana-biaya.index'))
        <div class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            @if ($rencana_biaya)
                <button onclick="window.location='{{ route('lahan.rencana-biaya.edit', [$lahan->lahan_id, $rencana_biaya->biaya_reklamasi_id]) }}?currency={{ $currency }}'" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" @disabled(!auth()->user()->can('update', [App\Models\BiayaReklamasi::class, $lahan]))>
            ...
            @else
                <button onclick="window.location='{{ route('lahan.rencana-biaya.create', $lahan->lahan_id) }}?tahun={{ $tahun_aktif }}&currency={{ $currency }}'" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" @disabled(!auth()->user()->can('create', [App\Models\BiayaReklamasi::class, $lahan]))>
        ...
        <div data-turbo="false" class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            <a href="{{ route('lahan.rencana-biaya.pdf', $lahan->lahan_id) }}" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200">
    ...
    @elseif (request()->routeIs('lahan.rencana-biaya.edit'))
        <div class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            <x-main.primary-button type="button" id="convertCurrencyBtn" class="cursor-pointer rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200">
                Convert Currency
            </x-main.primary-button>
        </div>
        <x-rencana-biaya.currency-dropdown :selected="$currency" />
        <div class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            <a href="{{ route('lahan.rencana-biaya.index', $lahan->lahan_id) }}?tahun={{ $tahun_aktif }}" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200">
    ...
    @elseif (request()->routeIs('lahan.rencana-biaya.create'))
        <x-rencana-biaya.currency-dropdown :selected="$currency" />
        <div class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            <a href="{{ route('lahan.rencana-biaya.index', $lahan->lahan_id) }}?tahun={{ $tahun_aktif }}" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200">
                <span class="relative leading-5 font-medium font-outfit">Back</span>
