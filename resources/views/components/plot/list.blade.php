                    ...
                    <tr>
                        ...
                        <td class="py-3 px-6 border-gainsboro border-t border-l">
                            <div class="flex flex-row items-center justify-center gap-3">
                                <button 
                                    type="button"
                                    class="border-none bg-transparent cursor-pointer p-0 disabled:opacity-50 disabled:cursor-not-allowed"
                                    x-on:click="window.location.href='{{ route('plot.show', $plot->plot_id) }}'"
                                    @disabled(!auth()->user()->can('view', $plot))
                                >
                                ...
                                <button 
                                    type="button"
                                    class="border-none bg-transparent cursor-pointer p-0 disabled:opacity-50 disabled:cursor-not-allowed"
                                    x-on:click="window.location.href='{{ route('plot.edit', $plot->plot_id) }}'"
                                    @disabled(!auth()->user()->can('update', $plot))
                                >
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-plot-deletion-{{ $plot->plot_id }}')" class="border-none bg-transparent cursor-pointer p-0 disabled:opacity-50 disabled:cursor-not-allowed" @disabled(!auth()->user()->can('delete', $plot))>
