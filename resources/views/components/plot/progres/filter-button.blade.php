...
<div class="relative inline-block gap-2">
    <a class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200 cursor-pointer"
        ...
        onclick="toggleProgresReklamasiFilterPanel()">
    ...
    <div id="progresReklamasiFilterPanel" class="absolute right-0 z-50 mt-2 p-4 bg-white border border-gray-300 rounded-lg shadow-lg hidden" style="min-width:350px;">
        <form id="filterForm" method="GET" action="{{ route('plot.show', $plot->plot_id) }}" class="space-y-3">
            <div>
                <x-main.input-label for="date" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Spesifik</x-main.input-label>
                <x-main.text-input type="date" id="date" name="date"
                       value="{{ request('date') }}"
