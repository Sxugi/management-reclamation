<div id="plotInfoModal" class="fixed inset-0 bg-black/50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between p-6 border-b border-gainsboro bg-gradient-to-r from-blue-50 to-indigo-50 rounded-t-2xl">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <div class="flex items-center self-stretch leading-7 font-semibold text-lg text-darkslategray font-outfit">Detail Data Progres Reklamasi</div>
                    <div class="text-sm text-slategray font-outfit">Informasi lengkap data progres reklamasi</div>
                </div>
            </div>
            <button onclick="window.closeDetailModal()" class="w-10 h-10 rounded-xl bg-white shadow-sm border border-gainsboro hover:bg-gray-500 flex items-center justify-center text-slategray hover:text-white transition-all duration-200">
        ...
        <div class="flex items-center justify-end p-6 border-t border-gainsboro bg-gray-50 rounded-b-2xl">
            <div class="flex items-center space-x-3">
                <x-main.primary-button onclick="window.closeDetailModal()" class="bg-red-500 !text-white text-sm py-3 px-4 rounded-lg font-medium hover:bg-red-600 transition-colors no-underline cursor-pointer">
                    Close
                </x-main.primary-button>
                <x-main.primary-button id="editButton" class="py-3 px-4 gap-2 font-medium cursor-pointer">
                    Edit
                ...
                <x-main.primary-button id="deleteButton" class="py-3 px-4 gap-2 font-medium hidden cursor-pointer">
                    Delete
...
<div id="imageModal" class="fixed inset-0 z-50 bg-black/75 flex items-center justify-center hidden p-4">
    <div class="relative max-w-4xl max-h-full p-4">
        <button onclick="closeImageModal()" 
                class="absolute top-6 right-6 rounded-lg border-none text-darkslategray-200 bg-white hover:text-white hover:bg-gray-600 transition-colors p-2">
