...
@php
    $rekapitulasi_reklamasi = $rekapitulasi_reklamasi[$tahun_aktif] ?? null;
@endphp

<div class="flex flex-row items-center justify-between gap-2 py-4 text-white">
    @if (request()->routeIs('lahan.rekapitulasi-reklamasi.index'))
        <div class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            @if ($rekapitulasi_reklamasi)
                <button onclick="window.location='{{ route('lahan.rekapitulasi-reklamasi.edit', [$lahan->lahan_id, $rekapitulasi_reklamasi->data_reklamasi_id]) }}'" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" @disabled(!auth()->user()->can('update', [App\Models\DataReklamasi::class, $lahan]))>
            ...
            @else
                <button onclick="window.location='{{ route('lahan.rekapitulasi-reklamasi.create', $lahan->lahan_id) }}?tahun={{ $tahun_aktif }}'" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" @disabled(!auth()->user()->can('create', [App\Models\DataReklamasi::class, $lahan]))>
        ...
        <div class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            <a data-turbo="false" href="{{ route('lahan.rekapitulasi-reklamasi.pdf', $lahan->lahan_id) }}?tahun={{ $tahun_aktif }}" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200">
    ...
    @else
        <div class="rounded-lg bg-darkslategray-300 overflow-hidden flex flex-row items-center justify-center gap-2">
            <a href="{{ route('lahan.rekapitulasi-reklamasi.index', $lahan->lahan_id) }}?tahun={{ $tahun_aktif }}" class="rounded-lg bg-darkslategray overflow-hidden flex flex-row items-center justify-center py-3 px-4 gap-2 !text-white no-underline hover:bg-slategray-200">
